...
public class GroupSettingService extends Service {
    ...
    public boolean deleteGroupFromSQLite(){
        boolean returnValue;
        queryForPeople();
        returnValue=deleteAndUniteGroup(deleteGroup_Recode,getString(R.string.default_group));
        return returnValue;
    }
    public boolean uniteGroupFromSQLite(){   //合并分组，先新建分组，再对原分组的数据进行操作
        ...
        returnValue=deleteAndUniteGroup(uniteGroup_Recode,newGroupName);
        return returnValue;
    }
    ...
    public void sendBroadcast2Main(){
        //向MainActivity发送更新视图的广播
        broadcastManager.sendBroadcast(this,1);
    }
    public void sendBroadcast2Category(){
        broadcastManager.sendBroadcast(this,4);
    }

    public boolean deleteAndUniteGroup(ArrayList<String> recode,String newgroupname){    //删除或合并分组都要进行的基本操作
